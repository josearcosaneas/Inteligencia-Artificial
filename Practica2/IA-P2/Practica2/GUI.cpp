...
const int AGENTE_REACTIVO=402,BUSQUEDA_ANCHURA=404,BUSQUEDA_COSTEUNIFORME= 405, BUSQUEDA_PROFUNDIDAD=403,ESCALADA=406;
...
bool pensado_plan[5]={false,false,false,false,false};
Plan plan[5];
...
GLUI_Button *agente_reactivo_btn,*display_btn,*busqueda_anchura_btn,*busqueda_coste_btn, *busqueda_profundidad_btn,*escalada_btn;
	...
	switch(id){
		...
		case DISPLAY_BTN_ID:
			if(env->DirtAmount()>0){
				...
				for(long long t=atol(display_step);t>0 and current_time<life_time and env->DirtAmount()>0;--t){
				    if (selected_option==0){
                      ...
                      int a = plan[selected_option].Siguiente_Accion();
			...
			if(env->DirtAmount()==0){
               ...
               escalada_btn->enable();
               plan[selected_option].EmpezarPlan();
           }
			break;
...
// Botones
		...
		busqueda_coste_btn=new GLUI_Button(main_glui,"B.C. Uniforme",BUSQUEDA_COSTEUNIFORME ,control_cb);
...
void GUI::showScore(){
	...
	switch(selected_option){
	   ...
	   case 2:
	           ...
	           break;
	   case 3:
	           sout<<"BUSQUEDA C. UNIFORME"<<"  , paso: "<<current_time;
	           time_step_text->set_text(sout.str().c_str());
	           break;
	   case 4:
	...
	if (selected_option==-1){
	  ...
	  complete_runs_text->set_text(sout.str().c_str());
	     ...
	     if (selected_option==0){
	      ...
	      sout<<"Longitud del Plan: "<<plan[selected_option].Get_Longitud_Plan();
	      complete_runs_text->set_text(sout.str().c_str());
	     }
	...
	if (selected_option==-1 or selected_option==0){
	  ...
	  sout<<"Nodos Expandidos: "<<plan[selected_option].Get_EstadosExpandidos()<<"      Nodos Explorados: "<<plan[selected_option].Get_EstadosExplorados();
	  total_score_text->set_text(sout.str().c_str());
      sout.str("");
	  sout<<"Energia Consumida: "<<plan[selected_option].Get_Energia();
	  average_score_text->set_text(sout.str().c_str());
	}
...
void GUI::AnchuraSeleccionado(){
  ...
  if (!pensado_plan[selected_option]){
    cout <<"Pensando ...." <<endl;
    plan[selected_option] = agent->Think(*env, selected_option);
    cout <<"Hecho!!" <<endl;
  }
  else {
    plan[selected_option].EmpezarPlan();
  ...
  display_btn->enable();
  life_time = plan[selected_option].Get_Longitud_Plan();
  pensado_plan[selected_option]=true;
}
...
void GUI::ProfundidadSeleccionado(){
  ...
  if (plan[selected_option].Get_Longitud_Plan()>0){
    /*agente_reactivo_btn->disable();
    busqueda_anchura_btn->disable();
    busqueda_profundidad_btn->disable();
    escalada_btn->disable();*/
